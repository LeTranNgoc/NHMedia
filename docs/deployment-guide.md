...
## 9.A Fly.io deploy (Singapore region) — actual commands

> Concrete steps for the closed-beta deploy. `fly.toml` + root `Dockerfile` already in repo; this section is the one-time setup + the recurring deploy flow.

### Prerequisites (~10 phút)
...
# Sign up + add payment method (free tier exists nhưng vẫn phải attach card)
fly auth signup
# hoặc nếu đã có account:
fly auth login
```

### Create app + provision (one-time, ~5 phút)

Chạy từ monorepo ROOT (`f:/translate voice/`):

```bash
# 1. Create the app (name in fly.toml is "translate-voice-backend" — change if taken)
fly apps create translate-voice-backend

# 2. Set all secrets (replace with real values from your local .env)
fly secrets set \
  MONGO_URI="mongodb+srv://<user>:<pass>@<cluster>.mongodb.net/translatevoice?retryWrites=true&w=majority" \
...
# 3. (Optional but recommended) Pin MongoDB Atlas via private network
# - Enable Atlas Network Peering OR add Fly's egress IPs to Atlas IP allowlist
# - Get Fly egress IPs:
fly ips list

# 4. GCP TTS service account key — upload as a secret file
fly secrets set GOOGLE_CLOUD_TTS_KEY_FILE="/app/secrets/gcp-tts.json"
# Then mount the actual file via volume OR inline JSON (smaller, easier):
fly secrets set GOOGLE_TTS_CREDENTIALS_JSON="$(cat backend/secrets/gcp-tts-service-account.json)"
# (And update backend/src/config/env.ts to read JSON env var as fallback — current code only reads file path; see follow-up below.)
```

### First deploy (~3-5 phút)

```bash
# Verify Docker build locally first (catches secrets / dockerignore issues before burning a Fly build)
docker build -t translate-voice-backend-local .
docker run --rm -p 3000:3000 --env-file backend/.env translate-voice-backend-local
# In another terminal:
curl http://localhost:3000/health  # → {"status":"ok"}
# Ctrl-C to stop, then deploy:

fly deploy
# Watch the build + first machine boot. Should land in ~3-5 min on first deploy.

# Verify:
curl https://translate-voice-backend.fly.dev/health
# → {"status":"ok"} or whatever your health-routes.ts returns
```
...
fly deploy           # rebuilds image, rolling deploy
...
# Roll back (Fly keeps last 3 releases):
fly releases list
fly releases rollback <version>
```

### Scale up later (when DAU > 300)

```bash
# Bump RAM
fly scale memory 1024

# Add a second region (lower latency for users outside SEA)
fly regions add nrt   # Tokyo
fly regions add hkg   # Hong Kong

# Add a machine in another region (manual horizontal scale)
fly machines create --region nrt --config fly.toml
```

### Follow-ups before first prod deploy

- [ ] **GCP TTS credentials shape:** `backend/src/config/env.ts` reads `GOOGLE_CLOUD_TTS_KEY_FILE` as a path. Fly secrets are env vars, not files. Options:
  - (a) Add a startup step in Dockerfile that writes `$GOOGLE_TTS_CREDENTIALS_JSON` → `/app/secrets/gcp-tts.json` then sets `GOOGLE_CLOUD_TTS_KEY_FILE=/app/secrets/gcp-tts.json`. Cleaner.
  - (b) Fly volume mounted at `/app/secrets`. More setup.
- [ ] **MongoDB connection pool:** default M0 caps at 500 connections; Fly machine + concurrent WS could spike. Upgrade to M10 when DAU > 100.
- [ ] **Health check route response format:** verify `/health` doesn't depend on MongoDB (Fly drains traffic if check fails — don't tie to a flaky downstream).
- [ ] **`CORS_ORIGINS`:** after Chrome Web Store publishes and gives production extension ID, run `fly secrets set CORS_ORIGINS=chrome-extension://<prod-id>`. The dev extension ID stays for continued testing — use comma-separated list.
- [ ] **Polar webhook URL:** in Polar dashboard, set webhook URL to `https://translate-voice-backend.fly.dev/billing/webhook`.
