...
**Coding level lookup:** read `codingLevel` from `.claude/.claude-config.json` (default `0` if missing). Env `CODING_LEVEL` overrides config. The same value drives the Tone Calibration injected at session start, so behavior stays consistent across the session.
