...
export class PipelineOrchestrator {
  ...
  private async _processChunk(srcText: string): Promise<void> {
    ...
    if (cached !== undefined) {
      ...
      try {
        translatedText = await this.translateProvider.translate(srcText, this.srcLang, 'vi');
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        this.emitter.emitError('translate_fail', message);
        // Emit subtitle with original text as fallback — no audio
        this.emitter.emitTranslation(srcText);
        return;
      }
